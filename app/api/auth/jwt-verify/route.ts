...
export const GET = async () => {
    try {
        ...
        if (!tokenCookie) {
            return NextResponse.json({ error: "Token manquant. Accès non autorisé", status: 401 });
        }
       ...
       return NextResponse.json({message: "Token validé succès", user: {id: user?.id, email: user?.email, username: user?.username, role: user?.role, avatar: user?.avatar}, valid: true,  status: 200});
